...
export const getAuthInfo = async () => {
  ...
  const url = buildUrl(
    config.baseUrl,
    `/cs/customer/v1/registration/loginAndUseMigration?view=LoginMini&_=${Date.now()}`
  );

  if (!url) {
    throw new Error(
      `Could not obtained auth info as the URL is not valid (URL: ${url})`
    );
  }
  ...
  const { headers } = await got(url, {
    headers: {
      ...
      cookie: 'localUser=',
      referer: buildUrl(config.baseUrl, '/home.html'),
      'user-agent':
